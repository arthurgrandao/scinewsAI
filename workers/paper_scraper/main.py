...
# Importações Locais
from workers.paper_scraper.modules.config import S2_API_KEY, PDF_STORAGE_PATH
from workers.paper_scraper.modules.network import get_robust_session
from workers.paper_scraper.modules.text_utils import extract_full_text_from_pdf, calculate_relevance_score
...
def run_curation_pipeline():
    ...
    try:
        for paper in winners:
            ...
            try:
                ...
                result = session_db.execute(stmt, db_entry)
                session_db.commit()

                # ON CONFLICT DO NOTHING returns rowcount=0 when article already exists.
                if result.rowcount and result.rowcount > 0:
                    ...
                    try:
                        ...
                        queued_count += 1
                        print("  [*] Enfileirado para tradução.")
                    except Exception as queue_error:
                        print(f"  [!] Falha ao enfileirar tradução: {queue_error}")
                else:
                    print("  [i] Artigo já existente, sem novo enfileiramento.")
    ...
    print(f"Total enfileirado para tradução: {queued_count}")
